...
openssl req -x509 -newkey rsa:4096 -keyout ca.key -out ca.crt -days 365 -nodes -subj "/CN=local.hierocracy.kubernetes-ca"
kubectl create secret tls ca-secret --key ca.key --cert ca.crt -n operators
...
echo ""
echo "csr describe"
kubectl describe csr $CERT_SERVICE_NAME.$CERT_SERVICE_NAMESPACE.cert

echo ""
echo "approving the cert"
kubectl certificate approve $CERT_SERVICE_NAME.$CERT_SERVICE_NAMESPACE.cert
...
kubectl get csr $CERT_SERVICE_NAME.$CERT_SERVICE_NAMESPACE.cert -o jsonpath='{.spec.request}' | \
  base64 --decode | \
...
kubectl get csr $CERT_SERVICE_NAME.$CERT_SERVICE_NAMESPACE.cert -o json | \
...
kubectl get csr $CERT_SERVICE_NAME.$CERT_SERVICE_NAMESPACE.cert -o jsonpath='{.status.certificate}' \
    | base64 --decode > server.crt

kubectl create secret tls server-cert --cert server.crt --key server-key.pem -n $CERT_SERVICE_NAMESPACE

kubectl create configmap example-serving-ca --from-file ca.crt=ca.pem -n $CERT_SERVICE_NAMESPACE
