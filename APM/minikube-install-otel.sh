...
echo "Create the minikube cluster, 'default'"
...
# For simplcity alias the kubectl calls to the minikube cluster call
# alias kubectl="minikube kubectl --"

# the linux session will need to have jq (jquery implementation) Installed
...
# if there is a current 'minikube' set up we are deleting it and rebuilding it
if [[ ${#miniKubeStatus[@]} -gt 0 ]]; then

	echo "THIS SCRIPT IS ABOUT TO DROP AND RECREATE THE 'minikube' cluster"
	read -n 1 -p "Hit any key to continue, ABORT THIS SCRIPT IF THE 'minikube' CLUSTER SHOULDN'T BE RECREATED AT THIS TIME"
fi
...
if [[ ${#miniKubeStatus[@]} -gt 0 ]]; then
 ...
 minikube start --cpus 8 --memory 8192m
read -n 1 -p "When k8s node is up hit any key"
echo ""

# set up the test service for deploy on the k8s cluster
# set the build context for docker to the cluster and 'docker build' it.
#  eval $(minikube docker-env)
# docker build ./ -t local/otel-test-service:1.0.0


# The grafana setup creates an ingress for the UI
...
echo "enabling minikube ingress-dns"
...
WaitForPodsRunning "operators" "cert-manager" 5
echo "Check the cert-manager operator deploy"
WaitForDeploymentToComplete "operators" "cert-manager-cainjector|cert-manager-webhook" 5
echo "Check the cert-manager service deploy"
WaitForServiceToStart "operators" "cert-manager" 5
echo "Check the cert-manager-webhook service deploy"
WaitForServiceToStart "operators" "cert-manager-webhook" 5
echo "Check the cert-manager-webhook-service service deploy"
WaitForServiceToStart "operators" "cert-manager-webhook-service" 5
...
echo ""
WaitForPodsRunning "elastic-system" "elasticsearch-es-" 15
echo ""
...
WaitForPodsRunning "observability" "jaeger-operator" 5
...
WaitForPodsRunning "observability" "jaeger-collector|jaeger-query" 5
echo ""
repeatToColWidth "+";
repeatToColWidth "+";
echo ""
...
WaitForPodsRunning "observability" "prometheus-" 5
...
WaitForPodsRunning "observability" "opentelemetry-" 5;
echo "check the opentelemetry-operator-webhook service"
WaitForServiceToStart "observability" "opentelemetry-operator-webhook" 5;
echo ""
...
WaitForPodsRunning "otlp-poc-test" "mysql" 15;
